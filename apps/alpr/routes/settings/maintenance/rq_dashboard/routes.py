...
def queues_overview(instance_number):
    r = make_response(
        render_template(
            ...
            segment='settings-maintenance-redis-queues',
        )
    )
...
def workers_overview(instance_number):
    r = make_response(
        render_template(
            "settings/rq_dashboard/workers.html",
            current_instance=instance_number,
            instance_list=current_app.config.get("RQ_DASHBOARD_REDIS_URL"),
            workers=Worker.all(),
            rq_url_prefix=url_for(".queues_overview"),
            ...
            segment='settings-maintenance-redis-workers',
        )
    )
    r.headers.set("Cache-Control", "no-store")
    return r
...
def jobs_overview(instance_number, queue_name, registry_name, per_page, page):
    ...
    r = make_response(
        render_template(
            "settings/rq_dashboard/jobs.html",
            current_instance=instance_number,
            instance_list=current_app.config.get("RQ_DASHBOARD_REDIS_URL"),
            queues=Queue.all(),
            queue=queue,
            ...
            segment='settings-maintenance-redis-jobs',
        )
    )
    r.headers.set("Cache-Control", "no-store")
    return r
...
def job_view(instance_number, job_id):
    ...
    r = make_response(
        render_template(
            "settings/rq_dashboard/job.html",
            current_instance=instance_number,
            instance_list=current_app.config.get("RQ_DASHBOARD_REDIS_URL"),
            id=job.id,
            rq_url_prefix=url_for(".queues_overview"),
            ...
            segment='settings-maintenance-redis-job',
        )
    )
    r.headers.set("Cache-Control", "no-store")
    return r
